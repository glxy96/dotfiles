...
Claude Code で使えるスラッシュコマンド：

| スキル | スコープ | 説明 |
|---|---|---|
| `/dotfiles-setup` | このリポジトリのみ | シンボリックリンクの作成・更新と実行権限付与（冪等）|
| `/branch-clean` | グローバル | 現在のブランチを安全チェック付きで削除 |
| `/branch-clean all` | グローバル | マージ済みブランチを一括削除 |
